...
function detectAvailableFilters() {
  ...
  tvSeriesData.titles.forEach(tvSeries => {
    if (tvSeries.startYear) hasYear = true;
    if (tvSeries.genres && tvSeries.genres.length > 0) hasGenre = true;
    if (tvSeries.animated !== undefined) hasAnimated = true;
    if (tvSeries.language) hasLanguage = true;
  });
